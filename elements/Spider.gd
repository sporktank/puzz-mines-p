...
func compute_actions():
    ...
    if not self.last_rotate and (
            (self.is_facing(self.w) and (self.n.is_blank() or self.n.is_lava() or self.n.is_player())) or
            (self.is_facing(self.s) and (self.w.is_blank() or self.w.is_lava() or self.w.is_player())) or
            (self.is_facing(self.e) and (self.s.is_blank() or self.s.is_lava() or self.s.is_player())) or
            (self.is_facing(self.n) and (self.e.is_blank() or self.e.is_lava() or self.e.is_player()))):
...
func animate_action(action, alpha):
    ...
    if action is Actions.Move or action is Actions.Drown:
        ...
        $AnimatedSprite.frame = fmod(alpha*3, 3)
        $AnimatedSprite.stop()
